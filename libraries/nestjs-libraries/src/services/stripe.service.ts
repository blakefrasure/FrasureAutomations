...
import { BillingSubscribeDto } from '@07ai/nestjs-libraries/dtos/billing/billing.subscribe.dto';
...
export class StripeService {
  ...
  async createOrGetCustomer(organization: Organization) {
    ...
    const customer = await stripe.customers.create({
      email: users.users[0].user.email.indexOf('@') > -1 ? users.users[0].user.email : `${users.users[0].user.email}@07ai.com`,
  ...
  private async createEmbeddedCheckout(
    ...
    try {
      await stripe.customers.update(customer, {
        email: user.email.indexOf('@') > -1 ? user.email : `${user.email}@07ai.com`,
        ...(body.dub
          ? {
              metadata: {
